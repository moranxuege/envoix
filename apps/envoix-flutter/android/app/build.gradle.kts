...
/**
 * The destination rule, as the CONSEQUENCE of the release channel rather than
 * its spelling: the Rust side owns what "public" implies and projects the one
 * environment name a channel demands, or an empty string when it demands none.
 * This build only compares two strings, so the two enforcers cannot come to
 * different conclusions about who an artifact may be handed to.
 */
val releaseChannel = policy("channel")
val requiredDestination = policy("destination_environment_required")
val requiredDestinationFlavor = policy("destination_flavor_required")
val compiledDeployment = policy("identity_deployment_environment")
...
fun verifyReleaseArtifact(
    ...
    if (compiledDeployment.isEmpty()) {
        failures += "$variant is a $releaseChannel release, but the build embeds no deployment identity"
    } else if (requiredDestination.isNotEmpty() && compiledDeployment != requiredDestination) {
        failures +=
            "$variant is a $releaseChannel release built for $compiledDeployment, " +
            "but a $releaseChannel release must be built for $requiredDestination"
    }
    if (requiredDestinationFlavor.isNotEmpty() && flavor != requiredDestinationFlavor) {
        failures +=
            "$variant is the \"$flavor\" flavour, but a $releaseChannel release " +
            "may only be the \"$requiredDestinationFlavor\" one"
    }
    ...
    logger.lifecycle(
        "release trust: $variant $kind judged ${entries.size} entries, ${payload.size} libraries, " +
            ...
            "destination $compiledDeployment on the $releaseChannel channel",
    )
}
