...
// ---------------------------------------------------------------------------
// LAN discovery record (parsed from mDNS TXT fields)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Discovery provider trait (legacy)
// ---------------------------------------------------------------------------
...
impl MdnsLanDiscovery {
    ...
    pub async fn discover(&self) -> Result<Vec<ConnectionCandidate>, DiscoveryError> {
        ...
        loop {
            ...
            // flume::Receiver::recv_async is an async channel receive.
            ...
            match event {
                Ok(Ok(ServiceEvent::ServiceResolved(info))) => {
                    if let Ok(record) = LanDiscoveryRecord::try_from_service_info(&info) {
                        ...
                        // Convert resolved addresses into candidates.
                ...
                Err(_timeout) => {
                    break; // timeout elapsed; stop collecting
                }
            }
        }
...
mod tests {
    ...
    fn missing_empty_session_id_is_rejected() {
        // Missing "sid" -> empty string -> should be rejected.
        let info = make_service_info(&[("proto", "1"), ("port", "9876")]);
        assert!(LanDiscoveryRecord::try_from_service_info(&info).is_err());

        // Explicitly empty "sid" -> also rejected.
    ...
    fn unknown_txt_keys_are_ignored() {
        let info = make_service_info(&[
            ...
            ("port", "9999"),
            ("features", "quic-v1"),
            ("token", "should-not-be-here"), // unknown key
            ("file", "secret.txt"),          // unknown key
        ]);
        let record = LanDiscoveryRecord::try_from_service_info(&info).unwrap();

        // Unknown keys are silently ignored – the record is still valid.
        assert_eq!(record.protocol_version, 1);
    ...
    fn sensitive_fields_are_not_emitted_by_serialization() {
        ...
        // These sensitive fields must never be serialized.
