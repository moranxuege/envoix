...
use crate::{
    ...
    RememberedSession, SenderManifestV2SessionSummary, SessionConfig, SessionError,
    TransferCancelToken, TransferEvent, bind_iroh_manifest_v2_endpoint,
...
struct AuthenticationMilestone<'a> {
    inner: &'a dyn AuthenticationHandler,
    authenticated: AtomicBool,
}

impl<'a> AuthenticationMilestone<'a> {
...
impl AuthenticationHandler for AuthenticationMilestone<'_> {
...
pub async fn send_manifest_v2_via_room_with_authentication(
    ...
    let authentication = AuthenticationMilestone::new(authentication);
