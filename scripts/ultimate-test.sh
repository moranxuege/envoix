...
device_hashes() {
    "$adb" -s "$SERIAL_B" shell \
        "find '$DEVICE_OUTPUT_DIR' -maxdepth 1 -type f -name 'ultimate-test-input*' -exec sha256sum {} \\; 2>/dev/null" |
        tr -d '\r'
}
...
run_test() {
    local environment="$1"
    local room="$(printf '%06d' $((RANDOM % 1000000)))-ultimate-test"
    local deadline
    local actual=""
    local app_uid
    local sender_state=""
    ...
    "$adb" -s "$SERIAL_B" shell \
        "find '$DEVICE_OUTPUT_DIR' -maxdepth 1 -type f -name 'ultimate-test-input*' -exec rm -f {} \\; 2>/dev/null || true"
    "$adb" -s "$SERIAL_A" push "$test_file" /data/local/tmp/ultimate-test-input >/dev/null
    app_uid="$("$adb" -s "$SERIAL_A" shell stat -c %u "/data/user/0/$PACKAGE" | tr -d '\r')"
    "$adb" -s "$SERIAL_A" shell \
        "cp /data/local/tmp/ultimate-test-input '$DEVICE_INPUT'; chown $app_uid:$app_uid '$DEVICE_INPUT'"
...
log_dir="$repo_root/android/build/ultimate-test"
mkdir -p "$log_dir"
