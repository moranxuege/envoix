...
use envoix_evidence::release::{
    BuildIdentity, Disagreement, Distribution, MeasuredArtifact, PackagedFacts, PayloadLibrary,
    PayloadRecord, ReleaseLedger, check_release, render_policy,
};
...
pub fn build_identity(root: &Path, ledger: &ReleaseLedger) -> CheckResult<BuildIdentity> {
    ...
    let payload = load_toml::<PayloadFile>(&root.join(PAYLOAD_FILE))?.payload;
    Ok(BuildIdentity {
...
pub fn record_payload(root: &Path) -> CheckResult<PayloadRecord> {
    ...
    let mut text = String::from(
        "# @generated by `cargo run -p xtask -- record-payload`, which\n\
         ...
         # Do not edit: this is the only accounting that ties the .so an APK\n\
         # packages to the manifest and the contract sources it was built from,\n\
         # and gradle never invokes cargo.\n\n[payload]\n",
    );
