...
impl UdpSocketState {
    pub fn new(sock: UdpSockRef<'_>) -> io::Result<Self> {
        ...
        Ok(Self {
            ...
            // Android network paths can accept the UDP_SEGMENT probe but return
            // EIO for real GSO sends. Avoid losing the first QUIC transmission.
            max_gso_segments: AtomicUsize::new(if cfg!(target_os = "android") {
                1
            } else {
                gso::max_gso_segments(&*io)
            }),
            gro_segments,
            may_fragment,
