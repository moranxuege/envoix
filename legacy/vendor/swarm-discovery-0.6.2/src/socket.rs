...
pub fn socket_v4(interface_addr: Option<Ipv4Addr>) -> Result<UdpSocket, SocketError> {
    ...
    // Patched: SO_REUSEPORT (Linux 3.9+) is best-effort; pre-3.9 kernels (e.g. a
    // 2.6.36 router) lack it, where skipping it only means no 5353 port-sharing.
    #[cfg(unix)]
...
pub fn socket_v6() -> Result<UdpSocket, SocketError> {
    ...
    // Patched: SO_REUSEPORT (Linux 3.9+) is best-effort; pre-3.9 kernels lack it.
    #[cfg(unix)]
    let _ = socket.set_reuse_port(true);
    socket
