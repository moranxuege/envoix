...
`values` / `values-b+zh+Hans` resources. The Activity container is migrated,
while its shared components and presentation formatters plus the remaining
transfer-setup content still contain inline bilingual text to be migrated.
